...
_PAIR_SAMPLES_PER_CHAR = 3  # random pair combinations per character
...
def build_training_lines(lang: str, chars: List[str]) -> List[str]:
    ...
    Uses real localized sentences where available, plus character-level
    exercises for special characters. No random gibberish.
    """
    ...
    # 1. Real localized subtitle sentences (the bulk of training data)
    sentences = LANG_SENTENCES.get(lang, [])
    for line in sentences:
        add(line)

    # 2. Individual special characters (universal set — music notes, symbols)
    for c in chars:
        add(c)

    # 3. Character pairs for special chars
    if len(chars) > 1:
        for c in chars:
            partners = rng.sample([x for x in chars if x != c],
                                  k=min(_PAIR_SAMPLES_PER_CHAR, len(chars) - 1))
            for p in partners:
                add(f"{c} {p}")

    # 4. Special characters in subtitle formatting contexts
    for c in chars:
        add(f"\u266a {c} \u266a")
        add(f'"{c}"')
        add(f"- {c}")
